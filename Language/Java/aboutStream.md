...
- 2중 for문의 첫 번째 Block 내부 for문의 두 번째 Block → 이러한 경우, `indent`가 2라고 말할 수 있다.
...
- `Stream생성().중개연산().최종연산();` 의 형태
- Stream은 `다양한 데이터소스`에서 생성 가능 → 컬렉션, 배열, 가변 매개변수, 람다 표현식, 빈 스트림 등

<br>

## 중개 연산

&nbsp; 중개 연산은 Stream을 전달 받아 Stream으로 반환하므로 중개 연산을 연속으로 사용할 수 있다. 또한 Stream의 중개 연산은 필터-맵(`filter-map`)기반의 API를 사용함으로 lazy 연산을 통해 `성능을 최적화`할 수 있다.
...
&nbsp; 위처럼, 예측했다면 peek() 중개연산을 `3번씩 총 2번, 6번 호출`한 것으로 이해할 수 있다.
...
&nbsp; 위에서의 연산은 peek() 중개 연산이 2개 있으니까 하나의 연산과정으로 병합하는 준비 과정이 진행된다.
